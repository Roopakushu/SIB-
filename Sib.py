...
CLIENT_SECRET_FILE = r"C:\Users\roopa\Documents\python_Automation\SIB\client_secret.json"  # Adjust path if needed
...
def get_latest_sheet():
    ...
    month_mapping = {month: index for index, month in enumerate(
        ["Jan", "Feb", "March", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], start=1)}
...
if latest_sheet_name:
    df = fetch_and_process_sheet(latest_sheet_name, "SIB")
    ...
    if df is None or df.empty:
        ...
        sql_query = f"INSERT INTO SIB ({', '.join(columns)}) VALUES ({placeholders})"
        ...
        try:
            ...
            conn.commit()
            print("✅ Data inserted into SIB table successfully!")
        except Error as e:
            conn.rollback()
